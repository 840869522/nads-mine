...
const ScenarioInstanceManagementPage: React.FC = () => {
    ...
    // 获取URL中的用户名参数
    const getUsernameFromUrl = useCallback(() => {
    ...
    const fetchInstances = useCallback(async () => {
        ...
        setError(null);
        try {
            // 根据URL参数决定是否过滤当前用户的场景
            const username = getUsernameFromUrl();
            // 使用专用API获取当前用户的场景实例
            const endpoint = username ? `/back/api/study/test/getUserScenarios?username=${encodeURIComponent(username)}` : '/back/api/scenariosinstances';
    ...
    // 初始加载和fetchInstances变化时重新获取数据
    useEffect(() => {
        fetchInstances();
    }, [fetchInstances]);

    // 监听URL中的username参数变化，以便在参数变化时重新获取数据
    useEffect(() => {
        const handleUrlChange = () => {
            fetchInstances();
        };
        
        // 添加hashchange事件监听，处理路由变化
        window.addEventListener('hashchange', handleUrlChange);
        
        return () => {
            window.removeEventListener('hashchange', handleUrlChange);
        };
    }, [fetchInstances, getUsernameFromUrl]);

    const handleRefresh = () => {
        // 确保使用当前URL中的用户名参数进行过滤
        fetchInstances();
    };
