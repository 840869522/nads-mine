...
export default function StoragePanel({ vmId }: StoragePanelProps) {
  ...
  const fetchDisks = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/back/api/vms/${vmId}/storage/disks`);
  ...
  const fetchCdRoms = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/back/api/vms/${vmId}/storage/cdroms`);
  ...
  const handleAddDisk = () => {
    ...
    handleApiCall(`/back/api/vms/${vmId}/storage/disks`, 'POST', { ...newDisk, capacity_gb: capacityNum }, () => {
  ...
  const handleDeleteDisk = (diskId: string) => {
    ...
    handleApiCall(`/back/api/vms/${vmId}/storage/disks/${diskId}`, 'DELETE');
  };
  ...
  const handleResizeDisk = () => {
    ...
    handleApiCall(`/back/api/vms/${vmId}/storage/disks/${diskToResize.id}/resize`, 'POST', { new_capacity_gb: sizeNum }, () => {
  ...
  const handleMountIso = () => {
    ...
    handleApiCall(`/back/api/vms/${vmId}/storage/cdroms/${cdRomToMount.id}/mount`, 'POST', { iso_path: selectedIsoPath }, () => {
      setMountIsoOpen(false);
    });
  };

  const handleEjectIso = (cdRomId: string) => {
    handleApiCall(`/back/api/vms/${vmId}/storage/cdroms/${cdRomId}/eject`, 'POST');
  };
