...
// 用户类型，ID 是字符串 (c_username)
interface User {
    u_id: string;   // 对应后端的 c_username
    u_name: string; // 也对应后端的 c_username
}
...
interface Team {
    ...
    score?: number;
    // 关键修改 1: 在整个前端组件中，我们统一使用 'members' 作为成员列表的键名
    members?: User[];
}
...
const Page: React.FC = () => {
    ...
    const fetchTeams = useCallback(async () => {
        ...
        try {
            ...
            // 后端返回 { data: [...] }, 每个 team 对象包含 users 数组
            ...
            const formattedTeams: Team[] = rawTeams.map((team: any) => ({
                ...
                score: team.score,
                // 然后，进行关键转换：将后端的 team.users 数组转换为前端的 team.members 数组
                members: team.users?.map((user: any) => ({
                    u_id: user.c_username,
                    u_name: user.c_username
                })) || [] // 如果 team.users 不存在，则提供一个空数组
            }));

            setTeams(formattedTeams); // 将完全格式化好的数据存入 state
    ...
    // fetchUsers 函数无需修改，它为 Autocomplete 提供独立的、扁平的用户列表
    const fetchUsers = useCallback(async () => {
        if (allUsers.length > 0) return;
        setIsUsersLoading(true);
        try {
            const response = await fetch('http://127.0.0.1:8000/api/ad/users');
    ...
    // === 事件处理器 (无需修改，因为它们依赖的是已经转换好的数据) ===
    const handleOpenForm = (team?: Team) => {
        fetchUsers();
        setEditingTeam(team || null);
        // 继续使用 team.members，因为数据在 fetchTeams 时已经转换好了
    ...
    const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const url = editingTeam ? `http://127.0.0.1:8000/api/ad/team/${editingTeam.c_id}` : 'http://127.0.0.1:8000/api/ad/team';
    ...
    const handleDeleteTeam = async () => {
        ...
        try {
            ...
            const result = await response.json();
            if (!response.ok) throw new Error(result.message || '删除队伍失败');
    ...
    // === 渲染逻辑 (无需修改，因为它们依赖的是已经转换好的、带有 'members' 属性的数据) ===
    return (
        <Box sx={{ p: 3, maxWidth: '1200px', margin: 'auto' }}>
                        ...
                        <Autocomplete
                            ...
                            renderInput={(params) => (
                                <TextField
                                    ...
                                    InputProps={{
                                        ...params.InputProps,
                                        endAdornment: (<>{isUsersLoading ? <CircularProgress color="inherit" size={20} /> : null}{params.InputProps.endAdornment}</>),
                                    }}
                                />
                            )}
                            sx={{ mt: 2 }}
            ...
            <Dialog open={isConfirmOpen} onClose={handleCloseConfirmDialog}>
                ...
                <DialogContent>
                    <Typography>您确定要删除队伍 "{teamToDelete?.c_name}" 吗？此操作无法撤销。</Typography>
                </DialogContent>
                <DialogActions>
