...
interface RuleItem {
  key?: string; // 仅用于编辑时回显，不允许手动输入
  tag: string;
  count: number;
  score: number;
  type: number; // 1:单选, 2:多选, 3:判断, 4:主观
}
    ...
    try {
      ...
      const transformedRules = rawRules.map((rule: any) => ({
        ...
        items: (Array.isArray(rule.items) ? rule.items : []).map((item: any) => ({
          key: item.key || item.c_id || '', // 仅回显，不允许编辑
    ...
    try {
      ...
      Object.entries(ruleData).forEach(([typeKey, itemData]: [string, any]) => {
        if (itemData && typeMap[typeKey as keyof typeof typeMap]) {
          items.push({
            key: String(itemData.key || itemData.c_id || ''), // 仅回显
...
const prepareRuleData = (rule: Rule) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const isUpdate = rules.some(r => r.testId === editingRule.testId);
