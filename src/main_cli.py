    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if os_node is not None:
                ...
                version = (
                    os_node.findtext("distro_version")
                    or os_node.findtext("distro-version")
                    or os_node.findtext("version")
                )
                if not version:
                    major = os_node.findtext("major_version") or os_node.findtext(
                        "distro_major_version"
                    )
                    minor = os_node.findtext("minor_version") or os_node.findtext(
                        "distro_minor_version"
                    )
                    if major:
                        version = major if not minor else f"{major}.{minor}"
                if distro:
                    meta["version"] = f"{distro} {version}" if version else distro
    except Exception:
        pass
    return meta
