...
{
    try {
        // 1. 新增：校验 c_start、c_end、c_duration 必传
        $requiredFields = ['c_experiment_name', 'c_config_id', 'c_start', 'c_end', 'c_duration'];
        ...
        // 校验时长为正整数且不超过起止时间差值（分钟）
        if (!is_numeric($data['c_duration']) || $data['c_duration'] < 1) {
            return [
                'code' => 422,
                'message' => 'c_duration must be a positive integer (minutes)',
            ];
        }
        ...
        if ($data['c_duration'] > $maxAllowedDuration) {
            return [
                'code' => 422,
                'message' => "c_duration cannot exceed {$maxAllowedDuration} minutes (difference between c_start and c_end)",
            ];
        }
        ...
        // 3. 核心修改：INSERT 语句添加 c_start、c_end、c_duration 字段和参数
        DB::beginTransaction();
        $result = DB::insert(
            'INSERT INTO c_course_experiments (
                c_experiment_id, c_course_id, c_experiment_name, c_description, 
                c_config_id, c_start, c_end, c_duration, created_at, updated_at
             ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())',
            [
                ...
                $data['c_duration']   // 8. 新增：c_duration
            ]
        );
...
public static function updateExperiment(string $courseId, string $experimentId, array $data): array
{
    try {
        ...
        // 3. 核心修改：UPDATE 语句添加 c_start、c_end、c_duration 字段和参数
        DB::beginTransaction();
        $result = DB::update(
            ...
            [
                ...
                $data['c_duration'],  // 6. 新增：c_duration
                $experimentId,        // 7. c_experiment_id（条件）
                $courseId             // 8. c_course_id（条件）
            ]
        );
