    ...
    private function checkSystemResources()
    {
        try {
            ...
            if ($memoryUsage > 85) {
                ...
                return response()->json(['message' => "启动失败：系统内存使用率 ({$memoryUsage}%) 超过 90% 的阈值。请联系管理员清理"], 503); // 503 Service Unavailable
            }
            ...
            if ($cpuUsage > 85) {
                ...
                return response()->json(['message' => "启动失败：系统CPU使用率 ({$cpuUsage}%) 超过 90% 的阈值。请联系管理员清理"], 503);
            }
